...
def seeCombinedError(error, p1, p2, dataset, signal_distro, min_samples, \
    ...
    """
    sampling_modes = list(error.keys())
    plt.gcf().clear()
    fig, ax = plt.subplots()
    font_size = 13
    ...
    x_axis = np.array(list(range(min_samples, max_samples, steps))) / dataset_size
    x_axis = np.log(x_axis)
    for m in sampling_modes:
        plt.plot(x_axis, np.log(error[m]), alpha=1.0)
        plt.fill_between(x_axis, np.log(p1[m]), np.log(p2[m]), alpha=0.2)
        plt.ylabel("Log of average scaled absolute error", fontsize=font_size)
